...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        KeyChainLog(@"设置了 短信验证码界面文本u框的代理");
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (string.length !=0) {//不是“删除”字符
        KeyChainLog(([NSString stringWithFormat:@"TF回调, 新增字符: %@", string]));
        ...
        if (textField.text.length <self.numberOfVertificationCode) {
            ...
            KeyChainLog(([NSString stringWithFormat:@"TF回调, 当前全部code: %@", self.vertificationCode]));
            if (self.label.text.length == _numberOfVertificationCode) {
                /******* 通过协议将验证码返回当前页面  ******/
                // 下一个 runloop 再进行 resign，防止被提前 resign，无法走 return 导致 textfiled 少一个字符
                KeyChainLog(([NSString stringWithFormat:@"TF回调, 当前code 够6位"]));
