...
static DMCallRecord *_DMCallRecords;
//static int otp_record_num;
//static int otp_record_alloc;
static int _DMRecordNum;
static int _DMRecordAlloc;
...
static inline uintptr_t pop_call_record() {
    ...
    if (cs->is_main_thread && _call_record_enabled) {
        ...
        if (cost > _min_time_cost && cs->index < _max_call_depth) {
            if (!_DMCallRecords) {
                _DMRecordAlloc = 1024;
                _DMCallRecords = malloc(sizeof(DMCallRecord) * _DMRecordAlloc);
            }
            _DMRecordNum++;
            if (_DMRecordNum >= _DMRecordAlloc) {
                _DMRecordAlloc += 1024;
                _DMCallRecords = realloc(_DMCallRecords, sizeof(DMCallRecord) * _DMRecordAlloc);
            }
            DMCallRecord *log = &_DMCallRecords[_DMRecordNum - 1];
            log->cls = pRecord->cls;
            log->depth = curIndex;
            log->sel = pRecord->cmd;
            log->time = cost;
...
void DMCallTraceStop() {
    _call_record_enabled = false;
}

void DMCallConfigMinTime(uint64_t us) {
    _min_time_cost = us;
}
void DMCallConfigMaxDepth(int depth) {
    _max_call_depth = depth;
}

DMCallRecord *DMGetCallRecords(int *num) {
    if (num) {
        *num = _DMRecordNum;
    }
    return _DMCallRecords;
}

void DMClearCallRecords() {
    if (_DMCallRecords) {
        free(_DMCallRecords);
        _DMCallRecords = NULL;
    }
