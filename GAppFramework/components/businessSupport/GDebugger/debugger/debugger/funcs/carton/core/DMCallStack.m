...
/// thread信息结构体
typedef struct DMThreadInfoStruct {
    /// CPU使用率
    double cpuUsage;
    ///
    integer_t userTime;
...
typedef struct DMStackFrame {
    const struct DMStackFrame *const previous;
    const uintptr_t return_address;
} DMStackFrame;

static mach_port_t _DMMainThreadId;
...
+ (NSString *)callStackWithType:(DMCallStackType)type {
    if (type == DMCallStackTypeMain) {
        // 主线程
        NSString *res = DMStackOfThread(_DMMainThreadId);
        assert(vm_deallocate(mach_task_self(), (vm_address_t)_DMMainThreadId, 1 * sizeof(thread_t)) == KERN_SUCCESS);
        return [res copy];
    } else if (type == DMCallStackTypeCurrent) {
        //当前线程
        char name[256];
        ...
        [nsthread setName:originName];
        reStr = DMStackOfThread(mach_thread_self());
        return [reStr copy];
    } else if (type == DMCallStackTypeAll) {
        // 线程数组 thread[1] = 5635
        ...
        for (int i = 0; i < thread_count; i++) {
            //当前执行的指令
            [reStr appendString:DMStackOfThread(threads[i])];
        }
        //task info
        NSString *memStr = @"";
...
NSString *DMStackOfThread(thread_t thread) {
    DMThreadInfoStruct threadInfoSt = {0};
    ...
    mach_msg_type_number_t state_count = DMThreadStateCountByCPU();
    kern_return_t kr = thread_get_state(thread, DMThreadStateByCPU(), (thread_state_t)&machineContext.__ss, &state_count);
    if (kr != KERN_SUCCESS) {
        return [NSString stringWithFormat:@"Fail get thread: %u", thread];
    }
    //通过指令指针来获取当前指令地址
    const uintptr_t instructionAddress = DMMachInstructionPointerByCPU(&machineContext);
    buffer[i] = instructionAddress;
    ++i;
    
    //
    uintptr_t linkRegisterPointer = DMMachThreadGetLinkRegisterPointerByCPU(&machineContext);
    ...
    DMStackFrame stackFrame = {0};
    //通过栈基址指针获取当前栈帧地址
    const uintptr_t framePointer = DMMachStackBasePointerByCPU(&machineContext);
    if (framePointer == 0 || DMMemCopySafely((void *)framePointer, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
        return @"Fail frame pointer";
    }
    for (; i < 32; i++) {
        buffer[i] = stackFrame.return_address;
        if (buffer[i] == 0 || stackFrame.previous == 0 || DMMemCopySafely(stackFrame.previous, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
            break;
        }
    }
    ...
    DMSymbolicate(buffer, symbolicated, stackLength, 0);
    for (int i = 0; i < stackLength; ++i) {
        [reStr appendFormat:@"%@", DMOutputLog(i, buffer[i], &symbolicated[i])];
    }
    [reStr appendFormat:@"\n"];
    return reStr;
}

NSString *DMOutputLog(const int entryNum, const uintptr_t address, const Dl_info* const dlInfo) {
...
void DMSymbolicate(const uintptr_t* const stackBuffer, Dl_info* const symbolsBuffer, const int stackLength, const int skippedEntries) {
    int i = 0;
    if (!skippedEntries && i < stackLength) {
        DMDladdr(stackBuffer[i], &symbolsBuffer[i]);
        i++;
    }
    for (; i < stackLength; i++) {
        DMDladdr(DMInstructionAddressByCPU(stackBuffer[i]), &symbolsBuffer[i]);
    }
}

kern_return_t DMMemCopySafely(const void *const src, void *const dst, const size_t byteSize) {
    vm_size_t bytesCopied = 0;
    return vm_read_overwrite(mach_task_self(), (vm_address_t)src, (vm_size_t)byteSize, (vm_address_t)dst, &bytesCopied);
}

bool DMDladdr(const uintptr_t address, Dl_info* const info) {
    info->dli_fname = NULL;
    info->dli_fbase = NULL;
    info->dli_sname = NULL;
    info->dli_saddr = NULL;
    //根据地址获取是哪个 image
    const uint32_t idx = DMDyldImageIndexFromAddress(address);
    if (idx == UINT_MAX) {
        return false;
    }
    /*
    ...
    const uintptr_t segmentBase = DMSegmentBaseOfImageIndex(idx) + imageVMAddressSlide;
    if (segmentBase == 0) {
        return false;
    }
    //
    ...
    uintptr_t cmdPointer = DMCmdFirstPointerFromMachHeader(machHeader);
    if (cmdPointer == 0) {
        return false;
    }
 ...
 */
uint32_t DMDyldImageIndexFromAddress(const uintptr_t address) {
...
uintptr_t DMCmdFirstPointerFromMachHeader(const struct mach_header* const machHeader) {
...
uintptr_t DMSegmentBaseOfImageIndex(const uint32_t idx) {
    const struct mach_header* machHeader = _dyld_get_image_header(idx);
    //查找 segment command 返回 image 的地址
    uintptr_t cmdPtr = DMCmdFirstPointerFromMachHeader(machHeader);
    if (cmdPtr == 0) {
        return 0;
    }
...
uintptr_t DMInstructionAddressByCPU(const uintptr_t address) {
 ...
 */
uintptr_t DMMachStackBasePointerByCPU(mcontext_t const machineContext) {
...
uintptr_t DMMachThreadGetLinkRegisterPointerByCPU(mcontext_t const machineContext) {
#if defined(__i386__)
    return 0;
#elif defined(__x86_64__)
    return 0;
...
uintptr_t DMMachInstructionPointerByCPU(mcontext_t const machineContext) {
...
thread_state_flavor_t DMThreadStateByCPU() {
#if defined(__arm64__)
...
mach_msg_type_number_t DMThreadStateCountByCPU() {
#if defined(__arm64__)
