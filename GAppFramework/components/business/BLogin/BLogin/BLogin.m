...
#import <DMHttpConfig/DMRequestUtil.h>
#import <DMLogin/DMLoginCoreLogic.h>
#import <DMConst/URDConst.h>
#import <DMUILib/DMHud.h>
...
- (void)pushToAgreementVC {
    [GRouter router:URDS(CSCHEME, URD_AGREEMENT)];
}
...
- (void)requestHeaderWithIsNeedC_UA:(BOOL)isNeedC_UA SuccessBlock:(void (^)(NSDictionary *))sBlock {
    [DMRequestUtil requestDefaultHeader].then(^id(GTaskResult<NSDictionary *> *t) {
        if (t.suc) {
            sBlock(t.data);
        }
        return nil;
...
+ (NSString *)requestBaseUrl {
    ...
    if (!cls) return @"https://jz-c.doumi.com";
