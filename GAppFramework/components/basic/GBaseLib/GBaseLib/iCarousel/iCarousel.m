...
@property (nonatomic, assign) CGFloat fixTrans;
...
- (CATransform3D)transformForItemViewWithOffset:(CGFloat)offset idx:(NSInteger)idx
{   
    ...
    switch (_type)
    {
        case iCarouselTypeCustom:
        {
            if ([_delegate respondsToSelector:@selector(carousel:itemTransformForOffset:baseTransform:scrollR:idx:)]) {
                return [_delegate carousel:self itemTransformForOffset:offset baseTransform:transform scrollR:self.rightS idx:idx];
            }
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    //calculate transform
    CATransform3D transform = [self transformForItemViewWithOffset:offset idx:index];
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gesture
{
    if ([gesture isKindOfClass:[UIPanGestureRecognizer class]])
    {
        ...
        {
            if (_positive && (_scrolling || _decelerating || _scrollOffset != [self currentItemIndex])) {
                return NO;
            }
            if (_vertical)
            {
...
//                _fixTrans = (_scrollOffset - self.currentItemIndex)*_itemWidth;
            ...
            case UIGestureRecognizerStateChanged:
            {
                ...
                velocity = -velocity;
#endif
                if (_positive && _fixTrans+translation>0) {
//                    if (!self.rightS && _previousTranslation * translation <0 && _fixTrans!=0) {
//                        _previousTranslation = -_previousTranslation;
//                    }
                    self.rightS = YES;
...
//                NSLog(@"==>%.2f ==>%.2f ==>%.2f ==>%.2f", _fixTrans, translation, _previousTranslation, _scrollOffset);
