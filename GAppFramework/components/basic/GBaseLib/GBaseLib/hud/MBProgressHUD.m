...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view type:(int)type animated:(BOOL)animated {
    MBProgressHUD *hud = [[self alloc] initWithView:view];
    hud.type = type;
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
        ...
        NSString *imgName = [NSString stringWithFormat:@"MBProgressHUD.bundle/loading-%@-1", self.type==1?@"3":@"2"];
        ...
        [self.indicator addSubview:img];
		[(UIActivityIndicatorView *)indicator startAnimating];
        
		[self addSubview:indicator];
        indicator.subviews[0].hidden = YES;
...
- (void)layoutSubviews {
    ...
    CGFloat maxHeigth = bounds.size.height - 4 * margin;
	CGSize totalSize = CGSizeZero;
	
	CGRect indicatorF = indicator.bounds;
	indicatorF.size.width = MIN(indicatorF.size.width, maxWidth);
	totalSize.width = MAX(totalSize.width, indicatorF.size.width);
    if (indicatorF.size.height > maxHeigth)
    {
        indicatorF.size.height = maxHeigth;
    }
...
//    CGSize labelSize = MB_TEXTSIZE(label.text, label.font);
    ...
    if (labelSize.height > maxHeigth)
    {
        labelSize.height = maxHeigth;
    }
	...
	CGFloat remainingDetailHeight = bounds.size.height - totalSize.height - kPadding - 4 * margin;
	CGSize maxDetailSize = CGSizeMake(maxWidth, remainingDetailHeight);
	CGSize detailsLabelSize = MB_MULTILINE_TEXTSIZE(detailsLabel.text, detailsLabel.font, maxDetailSize, detailsLabel.lineBreakMode);
    totalSize.width = MAX(totalSize.width, detailsLabelSize.width);
    if (detailsLabelSize.height > maxHeigth)
    {
        detailsLabelSize.height = maxHeigth;
    }
	...
	CGRect labelF;
    labelF.origin.y = yPos+([self.indicator isKindOfClass:[UIActivityIndicatorView class]]? 8 :0);
